...
from sacn.sacn_socket import merge_dict
...
def merge_inputs(sacn_data):
    if load_settings("sacn_to_artnet", "sACN") is True:
        if load_settings("merge_sacn", "sACN") is True:
            cid = sacn_data["cid"]
            universe = sacn_data["universe"]
            priority = sacn_data["priority"]
            dmx_data = sacn_data["dmx_data"]
            per_channel_priority = load_settings("per_channel_priority", "sACN")
            if cid not in merge_dict[universe]:
                merge_dict[universe].update({cid: {}})  # Create new entry for this CID if not already created
            if "priority" not in merge_dict[universe][cid]:
                # If per channel priority does not exist yet, add it.
                per_channel_priority = bytearray()  # Create empty bytearray
                for i in range(512):
                    per_channel_priority.append(priority)  # Copy universe priority to every channel
                merge_dict[universe][cid].update(priority=per_channel_priority)
            merge_dict[universe][cid].update(dmx=dmx_data, time=time())  # Update Time and DMX data
            output_dmx = flush_buffer(512)  # Reset DMX output to 0
            output_priority = flush_buffer(512)  # Reset Priority output to 0
            for cids in merge_dict[universe]:  # Loop for every CID input on this universe
                for dmx_length in range(512):  # Loop for every position of the DMX packet
                    if per_channel_priority is True:
                        # Merge mode can be changed in the UserParams.py
                        if output_priority[dmx_length] < merge_dict[universe][cids]["priority"][dmx_length]:
                            # If priority is higher, overwrite output.
                            output_priority[dmx_length] = merge_dict[universe][cids]["priority"][dmx_length]
                            if "dmx" not in merge_dict[universe][cids]:
                                pass
                                # Do not do anything if we receive a priority packet first.
                            else:
                                output_dmx[dmx_length] = merge_dict[universe][cids]["dmx"][dmx_length]
                                # Update DMX if priority is higher.
                        if output_priority[dmx_length] == merge_dict[universe][cids]["priority"][dmx_length]:
                            # If priority is equal, check DMX value
                            if "dmx" not in merge_dict[universe][cids]:
                                pass
                            elif output_dmx[dmx_length] < merge_dict[universe][cids]["dmx"][dmx_length]:
                                # If priority is equal, the highest value wins.
                                output_dmx[dmx_length] = merge_dict[universe][cids]["dmx"][dmx_length]
                    if per_channel_priority is False:
                        if "dmx" not in merge_dict[universe][cids]:
                            pass
                        elif output_dmx[dmx_length] < merge_dict[universe][cids]["dmx"][dmx_length]:
                            # If priority is equal, the highest value wins.
            ...
            for universes in merge_dict:
                # If a universe has timeout, remove from the dictionary, so it won't overwrite the priority of the other
                # active universes.
                for cids in merge_dict[universe]:
                    if time() - merge_dict[universe][cids]["time"] > E131_NETWORK_DATA_LOSS_TIMEOUT:
                        print(f"From {sacn_data['cid']} deleting universe {sacn_data['universe']}. Timeout after "
                              f"{time() - merge_dict[sacn_data['universe']][cids]['time']} seconds.")
                        del merge_dict[universe][cids]  # Delete the Universe on this CID and leave the loop.
                        break
            sacn_data["dmx_data"] = output_dmx
            sacn_data["per_channel_priority"] = output_priority
        elif load_settings("merge", "sACN") is False:
            pass
    # Store these data in the input input_dict and return
    return sacn_data["dmx_data"]


def add_per_channel_priority(sacn_data):
    cid = sacn_data["cid"]
    universe = sacn_data["universe"]
    if cid not in merge_dict[universe]:
        merge_dict[universe] = {cid: {}}
    ...
    merge_dict[universe][cid].update(priority=per_channel_priority, time=time())


def identify_startcode(sacn_input):
...
def identify_packet(sacn_input):
    # Identifies the universe_type of this sACN packet. Raises an error if the packet is too short.
    try:
        len(sacn_input) < 126
...
def validate_dmx_packet(sacn_packet):
    # E131 Data Packet:
    # # # ROOT LAYER # # #
...
def valicate_sync_packet(sacn_packet):
    # E131 Data Packet:
    # # # ROOT LAYER # # #
    # 0-1:      Preamble Size (0x0010)                                        <- Discard if not valid
    # 2-3:      Postable Size (0x0000)                                        <- Discard if not valid
    ...
    output = {"cid": sacn_packet[22:38], "sync_address": tuple(sacn_packet[45:47]),
              "sequence_number": sacn_packet[44]}
    return output


def validate_discovery_packet(sacn_packet):
    # E131 Data Packet:
    # # # ROOT LAYER # # #
    # 0-1:      Preamble Size (0x0010)                                        <- Discard if not valid
    # 2-3:      Postable Size (0x0000)                                        <- Discard if not valid
    ...
    # Dictionary with all the information we can get from this package
    output = {"cid": sacn_packet[22:38], "source_name": str(sacn_packet[44:108]), "page": tuple(sacn_packet[118]),
...
def sacn_rdm_input(sacn_packet):
    # If this is a RDM packet (Start Code = 0xCC)
    # Dictionary with all the information we can get from this package
    sacn_data = {"cid": sacn_packet[22:38], "source_name": str(sacn_packet[44:108]), "priority": sacn_packet[108],
                 "sync_address": tuple(sacn_packet[109:111]), "sequence_number": sacn_packet[111],
                 "option_flags": sacn_packet[112], "universe": tuple(sacn_packet[113:115]),
                 "start_code": sacn_packet[125],
                 "rdm_data": sacn_packet[126:638], "universe_hibyte": sacn_packet[113],
                 "universe_lobyte": sacn_packet[114]}
    return sacn_data


def sacn_alternate_input(sacn_packet):
    # If this is a alternate start code packet (Start Code != 0xDD, 0x00 and 0xCC)
    # Dictionary with all the information we can get from this package
    sacn_data = {"cid": sacn_packet[22:38], "source_name": str(sacn_packet[44:108]), "priority": sacn_packet[108],
                 "sync_address": tuple(sacn_packet[109:111]), "sequence_number": sacn_packet[111],
                 "option_flags": sacn_packet[112], "universe": tuple(sacn_packet[113:115]),
