...
from rdm.rdm_params import *
from load_settings import load_settings, save_settings
from artnet.artnet_socket import unicast_ips, artnet_unicast_socket, artnet_broadcast_socket
...
def artpollreply_output(target_ip='255.255.255.255', universe=0, ubea_version=0, indicator_state=00,
    ...
    good_output = int(f"{output_received}{output_test_packet}{output_sip}{output_text}{output_merging}{output_short}"
                      f"{output_ltp}{output_sacn}", 2)
...
def artdmx_output(artnet_data, target_ip="255.255.255.255", physical=0):
    ...
    sacndict = load_settings("sacndict")
    for universes in sacndict[universe_dec]:
...
def artvlc_output(artnet_data, target_ip="255.255.255.255", ieee=0, reply=0, beacon=0, transaction_number=0x0000,
    ...
    #           16: VlcModHi (Modulation universe_type number the transmitter should use, 0x0000 = use default value)
...
def arttoddata_output(artnet_data, target_ip="255.255.255.255", rdm_version="standard", port=1, bind_index=1,
                      command_response=0x00, uid_total=0, block_count=0, tod=None):
    # BROADCAST
    # 1:        ID[8] ('A''r''t''-''N''e''t' 0x00)
...
def artrdmsub_output(target_ip="255.255.255.255", rdm_version="standard", uid=0x000000000000,
    ...
    # 10:       ParameterIp (As per RDM specification. Defines the universe_type of parameter contained by this packet.)
    # 10:       Lo Byte of above
...
def artnet_output(artnet_packet, target_ip):
    delete_list = {}
    if load_settings("broadcast") is True:
        try:
            artnet_broadcast_socket.sendto(artnet_packet, (target_ip, load_settings("artnet_port", "Art-Net")))
        except Exception as exception:
            print(f"Socket error: {exception}")

    elif load_settings("broadcast") is False:
        if len(unicast_ips) > 40:  # If there are more than 40 receivers, send broadcast to save network capacity
            save_settings("Art-Net", "broadcast", True)
        for ips in unicast_ips:
            if time() - unicast_ips[ips]["time"] > ART_POLL_TIMEOUT:
                # Remove IP when there is a timeout
                print(f"Deleting {ips}. Timeout after {time() - unicast_ips[ips]['time']} seconds.")
                delete_list[ips] = unicast_ips[ips]
            try:
                artnet_unicast_socket.sendto(artnet_packet, ips)
                # print(f"Sending {artnet_packet} to {ips} as unicast")
            except Exception as exception:
                print(f"Socket error: {exception}")
        if delete_list:
